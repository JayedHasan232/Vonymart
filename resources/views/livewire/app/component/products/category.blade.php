<div>
    <a href="{{ route('short-url-dynamic-data-show', $category->url) }}" class="category"
        ...
        <div class="info d-none d-md-block">
            ...
            <span class="qty text-muted lh-1">{{ __( $category->products()->where('privacy', 1)->count() . ' Products')
                }}</span>
        </div>
    </a>
    <div class="mt-2 text-center d-block d-md-none">
        <div class="fs-4 text-dark lh-1">{{ __( $category->title ) }}</div>
        <div class="fs-6 text-muted lh-1">{{ __( $category->products()->where('privacy', 1)->count() . ' Products') }}
        </div>
    </div>
</div>